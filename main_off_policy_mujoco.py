...
if __name__ == "__main__":
	...
	parser.add_argument("--policy", default="SDPG")                   # Policy name
	parser.add_argument("--env", default="HalfCheetah-v2")           # OpenAI gym environment name
	parser.add_argument("--seed", default=0, type=int)               # Sets Gym, PyTorch and Numpy seeds
	parser.add_argument("--start_timesteps", default=25e3, type=int) # Time steps initial random policy is used
	parser.add_argument("--eval_freq", default=5e3, type=int)        # How often (time steps) we evaluate
	parser.add_argument("--max_timesteps", default=3e6+2, type=int)    # Max time steps to run environment
	...
	parser.add_argument("--exp_name", type=str,default="sdpg")       				 # Name for algorithms
	...
	if  not os.path.exists(logdir):
		os.makedirs(logdir)
	logger_kwargs = setup_logger_kwargs(args.exp_name, args.seed, datestamp=True,data_dir=logdir)
	...
	for t in range(int(args.max_timesteps)):
		...
		if t >= int(args.start_timesteps):
			if args.policy == 'SDPG':
				policy.train(_replay_buffer,t, args.batch_size)
			else:
		...
		if (t + 1) % args.eval_freq == 0:
			_agent(policy, eval_env, args.seed, logger)
			if args.save_model: 
			...
			# logger.log_tabular("entropy", policy.e)
